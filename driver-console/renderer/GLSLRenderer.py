...
class GLSLRenderer: 
    ...
    def __init__(self, dae):
        self.dae = dae
        self.pMatrix = [0.0]*16 # projection matrix
                ...
                for prim in geom.primitives():
                    ...
                    if triangles is not None:
                        ...
                        self.batch_list.append(
                            (batch, geom.original.id,shader_prog, tex_id, 
                             diff_color, spec_color, amb_color, shininess,
                             transform_matrix))
        self.updateProjectionMatrix(math.pi/3., 1)
        print 'done. Ready to render.'


    def updateProjectionMatrix(self, fovY, aspect, zNear = 0, zFar = 0):
        if zNear == 0:
            zNear = self.z_min
        if zFar == 0:
            zFar = self.z_max
        self.pMatrix = glutils.createProjectionMatrix(fovY, aspect, zNear / 2.0, zFar)
    ...
    def render(self, rotate_x, rotate_y, rotate_z):
        ...
        # Draw batches (VBOs)
        for (batch, mesh_id, shader_prog, tex_id, diff_color, spec_color, amb_color, shininess, transform_matrix) in self.batch_list:
            ...
            if shader_prog == self.shaders['phong']:
                shader_prog.uniform_matrixf('mvMatrix', transform_matrix)
                shader_prog.uniform_matrixf('pMatrix', self.pMatrix)

            # if mesh_id == 'steering_wheel-mesh':
            # if mesh_id == 'Plane-mesh':
                #glPushMatrix()
    ...
    def getTextureId(self, texture_name):
